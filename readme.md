...
Since we are a money transfer company this test will revolve around a (very) simplified transactionData model. Our aim is to implement the methods listed in `com.smallworld.TransactionDataFetcher`, a component that allows us to get some insight into the transactions our system has.
 ...
 * mtn: unique identifier of the transactionData
 * amount
 ...
 * issueId, issueSolved, issueMessage: issue information. Transactions can:
   ...
   * Contain a list of issues: in this case, the transactionData information will be repeated in different entries in `transactions.json` changing the issue related information.

You can think of the json as the output of an SQL query joining transactionData with transaction_issue, so the following example would represent a **unique transactionData** with **2 issues**:
```
{
    "mtn": 1284564,
    "amount": 150.2,
...
The parameters and return types of each method can be modified to fit the model that contains the transactionData information
